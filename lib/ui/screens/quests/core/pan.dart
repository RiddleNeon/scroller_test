...
class PanWidgetState extends State<PanWidget> {
  ...
  Quest? _findQuestInConnectZone(Offset scenePos) {
    ...
    for (final quest in questSystem.quests) {
      final handleCenter = Offset(
        quest.posX + quest.sizeX,
        quest.posY + quest.sizeY / 2,
      );
  ...
  void _onScaleStart(ScaleStartDetails details) {
    ...
    if (connectQuest != null) {
      ...
      _questBubbleOverlayKey.currentState?.setConnectionState(
        sourceId:   connectQuest.id,
        targetId:   null,
        previewPos: scenePos,
      );
      return;
    ...
    _questBubbleOverlayKey.currentState?.setDragState(
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (_isConnecting && _connectingFromQuest != null) {
      ...
      final targetId = (hoveredQuest != null &&
          hoveredQuest.id != _connectingFromQuest!.id)
          ? hoveredQuest.id
          : null;

      _questBubbleOverlayKey.currentState?.setConnectionState(
        sourceId:   _connectingFromQuest!.id,
        targetId:   targetId,
        previewPos: scenePos,
      );
      return;
  ...
  void _addConnection({required int sourceId, required int targetId}) {
    ...
    try {
      changeManager.record(AddConnectionChange(
        fromId: targetId,
        toId:   sourceId,
        updateMessage: 'connection drawn',
      ));
  ...
  void _onPointerScroll(PointerScrollEvent event) {
    ...
    final newScale = (currentScale *
        (1.0 - event.scrollDelta.dy * zoomSensitivity))
          ...
          child: QuestDetailScreen(
            ...
            onDoneEditing: (updatedQuest, [changeMessage]) async {
              await questRepo.upsertQuest(
                  updatedQuest, changeMessage ?? 'no message provided');
    ...
    return Listener(
      onPointerSignal: (e) {
        if (e is PointerScrollEvent) {
          final panelBox =
          _debugPanelKey.currentContext?.findRenderObject() as RenderBox?;
          ...
          child: Stack(
            children: [
              ...
              if (debugMode)
                QuestDebugPanel(
                  key:           _debugPanelKey,
                  onChanged:     () => _questBubbleOverlayKey.currentState?.refresh(),
                  onFocusQuest:  _focusOnQuest,
                ),
            ],
          ),
        ),
