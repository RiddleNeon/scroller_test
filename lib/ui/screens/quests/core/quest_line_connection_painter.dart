...
class QuestLineConnectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final quest in questSystem.quests) {
      ...
      /*for (final prereq in questSystem.prerequisitesOf(quest.id)) {
        ...
        final endQuest = quest;

        if (startQuest == null) continue;

        final startCenter = _centerOf(startQuest.id)!;
        final endCenter = _centerOf(endQuest.id)!;

        final pStart = _getEdgePoint(startQuest.id, endCenter);
        final pEnd = _getEdgePoint(endQuest.id, pStart);

        final cps = _cubicControlPoints(pStart, pEnd);
        final cp1 = cps[0];
        final cp2 = cps[1];
        ...
        final lut = _getLut(startCenter, cp1, cp2, endCenter);

        _drawBorderLine(canvas, startCenter, cp1, cp2, endCenter);
        _drawBaseLine(canvas, startCenter, cp1, cp2, endCenter, prereqColor, questColor);

        if (showArrows) {
          _drawMarchingArrows(
            canvas, startCenter, cp1, cp2, endCenter,
      ...
      for (final prereq in questSystem.prerequisitesOf(quest.id)) {
        ...
        // 1. Zentren für die Richtungsentscheidung bestimmen
        final startCenter = _centerOf(startQuest.id)!;
        final endCenter = _centerOf(quest.id)!;

        // 2. Beste Ankerpunkte finden (Source schaut zum Ziel-Zentrum, Target schaut zum Start-Zentrum)
        final anchorStart = _getBestAnchor(startQuest.id, endCenter);
        final anchorEnd = _getBestAnchor(quest.id, startCenter);

        // 3. Kontrollpunkte basierend auf den Richtungen der Anker berechnen
  ...
  List<Offset> _cubicControlPoints(Offset pStart, Offset dirStart, Offset pEnd, Offset dirEnd) {
    ...
    // Tension basierend auf der Distanz, damit kurze Wege nicht zu weit ausschwingen
    final tension = (dist * 0.4).clamp(30.0, 200.0);

    return [
      pStart + dirStart * tension, // Schussrichtung vom Start
      pEnd + dirEnd * tension,     // Ankunftsrichtung am Ziel
    ];
  }

  Offset _getEdgePoint(int id, Offset otherTarget, {bool isSource = true}) {
    final quest = questSystem.maybeGetQuestById(id);
    if (quest == null) return Offset.zero;

    final pos = (id == currentDraggedQuestId && currentDraggedQuestPos != null)
        ? currentDraggedQuestPos!
        : quest.position;

    final double centerX = pos.dx + quest.sizeX / 2;
    final double centerY = pos.dy + quest.sizeY / 2;

    if (otherTarget.dx < centerX) {
      return Offset(pos.dx, centerY);
    } else {
      return Offset(pos.dx + quest.sizeX, centerY);
    }
  }
