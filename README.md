...
## database provider
as for the database provider, ive descided to use **supabase**. supabase is a backend provider containing its own services like auth and third party connections, however ive descided to use firebase for that because of the reasons ive stated above.
...
firestore is a db provider in firebase. although it is a lot easier to connect to other firebase services, it has a lot of limitations. 
firestore uses **noSql**. noSql is a sql dialect like postgreSql, that stores its data a bit differently. ther are no tables you can access and connect, but instead there are collections, documents, subcollections and more. so if you want to access a users following for example, you would do something like this:
